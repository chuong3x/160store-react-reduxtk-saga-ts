  ...
  if (data.method === "email") {
    ...
    try {
      ...
      if (!emailIsExiting) {
        ...
        res.status(401).json({
          message:
            "Email đã được sử dụng. Nếu bạn quên mật khẩu, vui lòng tìm lại mật khẩu",
        });
      }
    } catch (err) {
    ...
    try {
      ...
      if (!isUserExit) {
        ...
        res.status(401).json({
          message:
            "Số điện thoại này đã được sử dụng. Nếu bạn quên mật khẩu, vui lòng tìm lại mật khẩu",
        });
      }
    } catch (err) {
      res.status(500).json({ error: err });
  ...
  if (data.method === "email") {
    ...
    try {
      ...
      if (!emailIsExiting) {
        ...
        res.status(200).json({
          message:
            "Email đã được dùng để đăng ký. Nếu bạn quên mật khẩu, vui lòng tìm lại mật khẩu",
        });
      }
    } catch (err) {
      res.status(500).json({ error: err });
    ...
    try {
      ...
      if (!phoneIsExiting) {
        ...
        res.status(200).json({
          message:
            "Số điện thoại này đã được dùng để đăng ký. Nếu bạn quên mật khẩu, vui lòng tìm lại mật khẩu",
        });
      }
    } catch (err) {
      res.status(500).json({ error: err });
      ...
      bcrypt.compare(password, user.password, async function (err, result) {
        if (!err) {
          if (result === true) {
            ...
            res.status(401).json({
              message:
                "Sai mật khẩu, vui lòng thử lại! Nếu bạn quên mật khẩu, vào quên mật khẩu để thực hiện tìm lại",
            });
          }
        }
      });
...
export const logout = async (req, res) => {
  ...
  try {
    ...
    if (user) {
      const { refreshTokens } = user;
      const newTokenList = refreshTokens.filter(
...
export const refreshToken = async (req, res) => {
  ...
  try {
    ...
    const { refreshTokens } = user;
    const removedOldToken = refreshTokens.filter(
      (token) => token !== refreshToken
    );
